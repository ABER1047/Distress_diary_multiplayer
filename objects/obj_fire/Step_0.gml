...
if check_n_room(id,code_m.my_ID)
{
	...
	timer ++
		...
		if timer > 6
		{
			...
			create_bubble_effect(irandom_range(10,100)/250,-90+irandom_range(-15,15),0,choose(-1)*irandom_range(1,100)/10,0.08,0.08,$FF00D2FE,$FF076BFE,1.5,$FF737272,x+irandom_range(-6,6),y-32,depth-1,1,true,false)
			}
	
			repeat(irandom_range(0,2))
			{
		
			create_bubble_effect(irandom_range(10,100)/250,-90+irandom_range(-15,15),0,choose(-1)*irandom_range(1,100)/10,0.07,0.07,c_white,$FF00D2FE,1.5,$FF342F32,x+irandom_range(-6,6),y-32,depth-1,1,true,false)
			}
